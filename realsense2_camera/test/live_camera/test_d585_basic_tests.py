...
machines that don't have the D585S connected.
...
class TestD585S_Change_Resolution(pytest_rs_utils.RsTestBaseClass):
    def test_D585S_Change_Resolution(self,launch_descr_with_parameters):
        ...
        try:
            ...
            assert self.set_string_param('rgb_camera.profile', '640x360x5')
...
@pytest.mark.launch(fixture=launch_descr_with_parameters)
class Test_D585S_Seq_ID_Update(pytest_rs_utils.RsTestBaseClass):
    def test_D585S_Seq_ID_update(self,launch_descr_with_parameters):
        params = launch_descr_with_parameters[1]
        if pytest_live_camera_utils.check_if_camera_connected(params['device_type']) == False:
            print("******************* Device not found? : " + params['device_type'])
            return
